...
use brain_core::types::{
    ...
    EvaluationResult, MasterOutput, MemoryStats,
};
...
impl Orchestrator {
    ...
    pub async fn query(&self, input: &str) -> Result<MasterOutput, String> {
        ...
        // --- 自动触发评估（使用真实快照） ---
        let snapshots: Vec<ContextSnapshot> = output.participating_brains.iter().map(|id| {
            ContextSnapshot {
                brain_id: id.clone(),
        ...
        if self
            .evaluation_brain
            .should_evaluate(&snapshots, 0, true)
        {
    ...
    pub fn evaluate_default(&self) -> String {
        ...
        let mut out = format!("=== 评估结果 ===\n  整体健康度: {:.2}\n", result.overall_health);
        for r in &result.brain_reports {
            use std::fmt::Write;
            let _ = writeln!(
                out,
                "  [{}] health={:.2} usage={:.0}%",
                r.brain_id, r.health_score, r.usage_percent * 100.0,
            );
        }
        out
    }
    ...
    pub async fn check_auto_dormancy(&self) -> Vec<BrainId> {
        ...
        if !dormant_ids.is_empty() {
            ...
            for id in &dormant_ids {
                let weight = weights.get(id).map_or(0.1, |w: &brain_core::types::Weight| w.value());
    ...
    pub async fn get_suggestions(&self) -> Vec<CreationSuggestion> {
        ...
        let candidates: Vec<Cand> = engine
            ...
            .map(|p| {
                let name = format!(
                    "auto-{}",
                    p.keywords.first().unwrap_or(&"unknown".into())
                );
                (
                    name,
                    p.keywords.clone(),
...
async fn handle_slow_think_dispatch<A: BrainAgent>(
    ...
    let response = {
        ...
        // 用缓存的广播消息或从协作消息重建
        let broadcast = last_broadcast
            .cloned()
            .unwrap_or_else(|| BroadcastMessage {
        ...
        if !related_memories.is_empty() {
            ...
            for mem in &related_memories {
                tracing::debug!("  记忆: {} (importance={:.2})", mem.content.chars().take(60).collect::<String>(), mem.importance);
            }
        }
...
/// 创建所有副脑
fn create_sub_brains(
) -> Result<
    (MemoryBrain, ReasoningBrain, MotorBrain, ValidationBrain, EvaluationBrain),
    String,
> {
    ...
    let motor = MotorBrain::new(MotorConfig::default())
        .map_err(|e| format!("执行脑初始化失败: {e}"))?;
    let validation = ValidationBrain::new(ValidationConfig {
        truthfulness_warning_threshold: 0.60,
    })
    .map_err(|e| format!("校验脑初始化失败: {e}"))?;
    let evaluation = EvaluationBrain::with_defaults()
        .map_err(|e| format!("评估脑初始化失败: {e}"))?;
...
mod tests {
    ...
    async fn test_orchestrator_init() {
        ...
        assert_eq!(orch.task_count(), 4, "应该有 4 个副脑任务");
    }
