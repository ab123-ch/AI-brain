...
impl PyramidMemoryBrain {
    ...
    #[must_use]
    fn novel_memory_store(&self) -> crate::novel::NovelMemoryStore {
    ...
    fn novel_lifecycle_store(&self) -> crate::novel::NovelLifecycleStore {
        crate::novel::NovelLifecycleStore::new(self.config.base_dir.clone())
    }
    ...
    pub fn load_novel_project(&self, project_id: &str) -> Result<crate::novel::NovelProject> {
        self.novel_memory_store().load_project(project_id)
    }

    pub fn list_novel_projects(&self) -> Result<Vec<crate::novel::NovelProject>> {
        self.novel_memory_store().list_projects()
    }
    ...
    pub fn append_novel_task_event(&self, event: &crate::novel::NovelTaskEvent) -> Result<()> {
        self.novel_lifecycle_store().append_event(event)
    }

    pub fn load_novel_task_events(
        &self,
        task_id: &str,
    ) -> Result<Vec<crate::novel::NovelTaskEvent>> {
        self.novel_lifecycle_store().load_events(task_id)
    }

    pub fn save_novel_task_checkpoint(
        &self,
        checkpoint: &crate::novel::NovelTaskCheckpoint,
    ) -> Result<()> {
        self.novel_lifecycle_store().save_checkpoint(checkpoint)
    }

    pub fn load_novel_task_checkpoint(
        &self,
        task_id: &str,
    ) -> Result<Option<crate::novel::NovelTaskCheckpoint>> {
        self.novel_lifecycle_store().load_checkpoint(task_id)
    }

    pub fn active_novel_task_for_project(
        &self,
        project_id: &str,
    ) -> Result<Option<crate::novel::NovelTaskCheckpoint>> {
        self.novel_lifecycle_store()
            .active_checkpoint_for_project(project_id)
    }

    pub fn begin_novel_publication(
        &self,
        record: &crate::novel::NovelPublicationRecord,
    ) -> Result<()> {
        let project = self.load_novel_project(&record.project_id)?;
        if record.project_id != record.delta.project_id {
            return Err(crate::error::MemoryError::Conflict(
                "发布项目与 NovelMemoryDelta 项目不一致".into(),
            ));
        }
        if record.expected_revision != project.canon_revision
            || record.delta.expected_revision != project.canon_revision
        {
            return Err(crate::error::MemoryError::Conflict(format!(
                "小说项目 revision 已变化: expected={}, actual={}",
                record.expected_revision, project.canon_revision
            )));
        }
        if record.output_path.trim().is_empty() || record.content_sha256.trim().is_empty() {
            return Err(crate::error::MemoryError::Conflict(
                "发布路径和内容 hash 均不能为空".into(),
            ));
        }
        self.novel_lifecycle_store().begin_publication(record)
    }

    pub fn load_novel_publication(
        &self,
        publication_id: &str,
    ) -> Result<crate::novel::NovelPublicationRecord> {
        self.novel_lifecycle_store()
            .load_publication(publication_id)
    }

    pub fn complete_novel_publication(
        &self,
        publication_id: &str,
        artifact: crate::novel::NovelArtifactReceipt,
    ) -> Result<crate::novel::CommitReport> {
        let store = self.novel_lifecycle_store();
        let mut record = store.load_publication(publication_id)?;
        if record.status == crate::novel::NovelPublicationStatus::Completed {
            return record.commit_report.ok_or_else(|| {
                crate::error::MemoryError::Conflict("已完成发布缺少 Canon commit report".into())
            });
        }
        if !matches!(
            record.status,
            crate::novel::NovelPublicationStatus::Pending
                | crate::novel::NovelPublicationStatus::ArtifactSaved
        ) {
            return Err(crate::error::MemoryError::Conflict(format!(
                "发布事务 {} 当前状态不允许完成: {:?}",
                record.publication_id, record.status
            )));
        }
        if artifact.canonical_path != record.output_path || artifact.sha256 != record.content_sha256
        {
            return Err(crate::error::MemoryError::Conflict(
                "ArtifactReceipt 与已审核发布内容不匹配".into(),
            ));
        }

        record.artifact = Some(artifact);
        record.status = crate::novel::NovelPublicationStatus::ArtifactSaved;
        record.updated_at = chrono::Utc::now().timestamp_millis();
        store.save_publication(&record)?;

        let report = match self
            .novel_memory_store()
            .commit_delta_for_publication(&record.delta, publication_id)
        {
            Ok(report) => report,
            Err(error) => {
        ...
        record.status = crate::novel::NovelPublicationStatus::Completed;
        record.commit_report = Some(report.clone());
        record.error = None;
        record.updated_at = chrono::Utc::now().timestamp_millis();
        store.save_publication(&record)?;
        Ok(report)
    ...
    pub fn abort_novel_publication(&self, publication_id: &str, reason: &str) -> Result<()> {
        ...
        record.status = crate::novel::NovelPublicationStatus::Aborted;
        record.error = Some(reason.to_string());
        record.updated_at = chrono::Utc::now().timestamp_millis();
        store.save_publication(&record)
    }

    pub fn pending_novel_publications(&self) -> Result<Vec<crate::novel::NovelPublicationRecord>> {
        self.novel_lifecycle_store().pending_publications()
    }
...
mod tests {
    ...
    #[test]
    fn novel_publication_flow_records_artifact_and_commits_canon_once() {
        use crate::novel::{
            CanonStatus, NovelArtifactReceipt, NovelFactKind, NovelMemoryDelta, NovelProject,
            NovelPublicationRecord, NovelTaskType, ProposedFact,
        };

        let tmp = tempfile::tempdir().unwrap();
        let brain = make_brain(&tmp);
        brain
            .create_novel_project(&NovelProject::new("dark-city", "暗城"))
            .unwrap();
        let delta = NovelMemoryDelta {
            project_id: "dark-city".into(),
            branch_id: "main".into(),
            expected_revision: 0,
        ...
        brain.begin_novel_publication(&record).unwrap();
        assert_eq!(brain.pending_novel_publications().unwrap().len(), 1);

        let artifact = NovelArtifactReceipt {
        ...
        let report = brain
            .complete_novel_publication("publication-1", artifact.clone())
            .unwrap();
        let retry = brain
            .complete_novel_publication("publication-1", artifact)
            .unwrap();

        assert_eq!(report.new_revision, 1);
        assert_eq!(retry.new_revision, 1);
        assert!(brain.pending_novel_publications().unwrap().is_empty());
        let project = brain.load_novel_project("dark-city").unwrap();
